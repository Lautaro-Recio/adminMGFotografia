...
export default function SwiperMod(props) {
    const { WhoMod,book,classOfContainer,classOfPics,setImageAndBook,getData } = {...props}
  return (
    <>
                  {/* WhoMod, book , classOfContainer, getData, classOfPics, setImageAndBook*/}
                    <div className={WhoMod == book.bookName ? classOfContainer: "opacity-0 h-0 transition"}> 
                      <Swiper  modules={[Navigation, Pagination]} spaceBetween={10} navigation pagination={{ clickable: true }} slidesPerView={book.imgs.book.length <= 4 ? book.imgs.book.length+1 : 4 } >
                        ...
                        { book.imgs.book.map( img =>{
                          ...
                          <>
                            <SwiperSlide  key={img.img.result}>
                              <div className={`w-40 h-40 mx-2 transition-all duration-700  ${WhoMod == book.bookName ? "opacity-100":"opacity-0"} `} >
                                <img className="w-40 h-40 rounded-md  border-2 border-borderGray " src={img.img.result} alt={img.result} />
