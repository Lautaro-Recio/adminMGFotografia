...
import SwiperMod from "./SwiperMod";
  export default function Module(props) {
      ...
      const openModule = (e,nameOfBook)=>{
          e.preventDefault()
          setClassOfContainer("opacity-100 flex duration-[2500ms] transition-all mx-2 h-56 ")
          setClassOfPics("opacity-100 flex duration-[2500ms] transition-all mx-2 h-[172px] w-[172px]")
    ...
    return (
      ...
      {BooksOnDB.map(book =>{
          return(
              ...
              <div className={`block my-2 rounded-md items-center px-2 border-2 border-Gray origin-top duration-700 transition-all ${WhoMod == book.bookName ? "h-80":"h-16"}`} key={book.bookName}>
                <div className="flex items-center justify-between my-2 mx-4">
                  <div className="flex">
                      ...
                      {WhoMod === book.bookName ?(
                              <button onClick={(e)=>closeModule(e)} className="flex justify-center items-center w-10 h-10  text-3xl hover:bg-whte rounded-full hover:text-menuGray transition-all"><ion-icon name="close-outline" ></ion-icon></button>
                          ):(
                ...
                {WhoMod == book.bookName &&
                  <SwiperMod WhoMod={WhoMod} book={book} classOfContainer={classOfContainer} getData={getData} classOfPics={classOfPics} setImageAndBook={setImageAndBook}/>
                }
              </div>
            )
          })
